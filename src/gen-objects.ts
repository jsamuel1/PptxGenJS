			...
			row.forEach((cell: number | string | TableCell) => {
				...
				arrSides.forEach(idx => {
					newCell.options.border[idx] = {
						...
						transparency: newCell.options.border[idx].transparency,
					}
				})
	...
	if (typeof opt.border === 'string') {
		...
		[0, 1, 2, 3].forEach(idx => {
			opt.border[idx] = opt.border[idx]
				? { type: opt.border[idx].type || DEF_CELL_BORDER.type, color: opt.border[idx].color || DEF_CELL_BORDER.color, pt: opt.border[idx].pt || DEF_CELL_BORDER.pt, transparency: opt.border[idx].transparency }
				: { type: 'none' }
		})
	}
	...
	*/
	...
	if (opt && !opt.autoPage) {
		...
		getSlidesForTableRows(arrRows, opt, presLayout, slideLayout).forEach((slide, idx) => {
			...
			if (idx > 0) opt.y = inch2Emu(opt.autoPageSlideStartY || arrTableMargin[0])
...
export function addTextDefinition(target: PresSlide, text: TextProps[], opts: TextPropsOptions, isPlaceholder: boolean): void {
	...
	function cleanOpts(itemOpts: ObjectOptions): TextPropsOptions {
		// STEP 1: Set some options
		{
			...
			// E: Transform underline boolean shorthand to object form
