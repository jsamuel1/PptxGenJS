 ...
 */
export function handoutMasterRid (slides: PresSlide[], embeddedFonts?: EmbedFontProps[]): number {
 ...
 */
function commentDateToIso (date?: Date | string): string {
	let d: Date
 ...
 */
export function collectCommentAuthors (slides: PresSlide[]): Map<string, { id: number, initials: string, count: number }> {
	const authors = new Map<string, { id: number, initials: string, count: number }>()
	let nextId = 0
...
/**
 * True when any slide carries review comments — gates all comment parts/rels/overrides (default-off).
 * @param {PresSlide[]} slides - presentation slides
 */
export function hasAnyComments (slides: PresSlide[]): boolean {
	return (slides || []).some(s => (s._comments || []).length > 0)
}
 ...
 * @return {string} XML
 */
export function makeXmlCommentAuthors (slides: PresSlide[]): string {
...
function genXmlTextRunProperties (opts: ObjectOptions | TextPropsOptions, isDefault: boolean): string {
	...
	if (opts.hyperlink) {
		...
		if (opts.hyperlink.url) {
			...
			runProps += `<${hlinkTag} r:id="rId${opts.hyperlink._rId}" invalidUrl="" action="" tgtFrame="" tooltip="${opts.hyperlink.tooltip ? encodeXmlEntities(opts.hyperlink.tooltip) : ''
 ...
 * @returns XML
 */
export function makeXmlContTypes (slides: PresSlide[], slideLayouts: SlideLayout[], masterSlide?: PresSlide, embeddedFonts?: EmbedFontProps[], hasHandoutMaster?: boolean): string {
 ...
 * @returns XML
 */
export function makeXmlPresentationRels (slides: PresSlide[], embeddedFonts?: EmbedFontProps[], hasHandoutMaster?: boolean): string {
	let intRelNum = 1
	let strXml = '<?xml version="1.0" encoding="UTF-8" standalone="yes"?>' + CRLF
 ...
 * @returns {string} XML
 */
export function makeXmlNotesMaster (headerFooter?: HeaderFooterProps): string {
	...
	if (headerFooter) {
		...
		if (hdr) hdrTextXml = `<a:p><a:r><a:rPr lang="en-US"/><a:t>${encodeXmlEntities(headerFooter.header as string)}</a:t></a:r></a:p>`
		if (ftr) ftrTextXml = `<a:p><a:r><a:rPr lang="en-US"/><a:t>${encodeXmlEntities(headerFooter.footer as string)}</a:t></a:r></a:p>`
	}
...
export function makeXmlSlideRel (slides: PresSlide[], slideLayouts: SlideLayout[], slideNumber: number): string {
	const defaultRels: Array<{ target: string, type: string }> = [
		{
 ...
 * @returns {string} XML
 */
export function makeXmlHandoutMaster (props?: HandoutMasterProps): string {
	let hfXml = ''
	let hdrTextXml = '<a:p><a:endParaRPr lang="en-US"/></a:p>'
	let ftrTextXml = '<a:p><a:endParaRPr lang="en-US"/></a:p>'
	const hf = props?.headerFooter
	if (hf) {
		const sldNum = hf.slideNumber ? 1 : 0
		...
		hfXml = `<p:hf sldNum="${sldNum}" hdr="${hdr}" ftr="${ftr}" dt="${dt}"/>`
		if (hdr) hdrTextXml = `<a:p><a:r><a:rPr lang="en-US"/><a:t>${encodeXmlEntities(hf.header as string)}</a:t></a:r></a:p>`
		if (ftr) ftrTextXml = `<a:p><a:r><a:rPr lang="en-US"/><a:t>${encodeXmlEntities(hf.footer as string)}</a:t></a:r></a:p>`
	}
	// Background: explicit hex solid fill when provided, else the theme background reference.
	const bgXml = typeof props?.background === 'string' && props.background.length > 0
 ...
 * @returns {string} XML
 */
export function makeXmlHandoutMasterRel (): string {
	return `<?xml version="1.0" encoding="UTF-8" standalone="yes"?>${CRLF}<Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships">
		<Relationship Id="rId1" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/theme" Target="../theme/theme2.xml"/>
		</Relationships>`
}
