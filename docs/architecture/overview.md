...
## Public surface (three entries)

`package.json#exports` publishes three entries, each with hand-maintained
...
| `./icons-fa` | `src/icons-fa.ts` | Font Awesome icon-path pack (~2 MB), lazy-loaded on demand |
...
| `icons-fa.ts` (+ `.generated.ts`) | generated FA pack behind the lazy subpath entry |
...
`subset-icon-pack.ts`, `tokenize-code.ts`.
