...
- Photo album — `pptx.photoAlbum = { blackWhite?, showCaptions?, layout?, frame? }` emits a `<p:photoAlbum>` metadata element into `presentation.xml` in its canonical `CT_Presentation` child position (after `<p:custShowLst>`, before `<p:kinsoku>`). `blackWhite` and `showCaptions` map to the `bw`/`showCaptions` attributes (boolean → `"1"`/`"0"`) and are always emitted; `layout` (`'fitToSlide' | '1pic' | '2pic' | '4pic' | '1picTitle' | '2picTitle' | '4picTitle'`) and `frame` (`'frameStyle1'`…`'frameStyle7'`) are emitted only when set (schema defaults apply otherwise). Metadata only — image slides are still authored normally. Default-off: a presentation that never sets `photoAlbum` emits byte-identical `presentation.xml`.
- Custom shows — `pptx.addCustomShow({ name, slides })` defines named, ordered subsets of the deck's slides, emitted as a `<p:custShowLst>` element in `presentation.xml` in its canonical `CT_Presentation` child position (after `<p:notesSz>`, before `<p:kinsoku>`/`<p:defaultTextStyle>`). Each show emits `<p:custShow name="…" id="N">` with a 0-based sequential `id`, containing a `<p:sldLst>` of `<p:sld r:id="rIdN"/>` entries that reuse each included slide's existing presentation relationship id (the same `r:id` used by its `<p:sldId>`). The show `name` is XML-escaped via `encodeXmlEntities`; calls missing `name` or a non-empty `slides` array warn and no-op. Default-off: a presentation that never calls `addCustomShow()` emits byte-identical `presentation.xml`.
- Kinsoku (East-Asian line-break) rules — `pptx.kinsoku = { lang?, invalStChars?, invalEndChars? }` emits a `<p:kinsoku lang invalStChars invalEndChars/>` element into `presentation.xml` in its canonical `CT_Presentation` child position (after `<p:notesSz>`, immediately before `<p:defaultTextStyle>`). `lang` defaults to `'ja-JP'`; `invalStChars`/`invalEndChars` (the characters that may not start/end a line in CJK typography) default to a representative Japanese set, so most users need only set `lang`. All three attribute values are XML-escaped via `encodeXmlEntities`. Default-off: a presentation that never sets `kinsoku` emits byte-identical `presentation.xml`.
- Navigation action jumps — `hyperlink.action` on `addText()`/`addShape()`/`addImage()` adds interactive next/previous/first/last/end-show navigation buttons. `action: 'nextSlide' | 'prevSlide' | 'firstSlide' | 'lastSlide' | 'endShow'` emits `<a:hlinkClick r:id="" action="ppaction://hlinkshowjump?jump=<verb>"/>` (verb map: `nextSlide→nextslide`, `prevSlide→previousslide`, `firstSlide→firstslide`, `lastSlide→lastslide`, `endShow→endshow`) with **no relationship** (empty `r:id`). `action: 'slide'` (combined with `slide: N`) reuses the existing slide-jump relationship path (`ppaction://hlinksldjump`) unchanged. Navigation actions allocate no slide relationship and log no error; `action` and `url` are mutually exclusive (if both are set, `url` wins with a `console.warn`). Works with `on: 'hover'` (the element name still swaps per parent type). Default-off: hyperlinks with no `action` emit byte-identical XML.
- Hover hyperlinks — `hyperlink.on` on `addText()`/`addShape()`/`addImage()` selects the hyperlink activation trigger: `'click'` (default, unchanged → `<a:hlinkClick>`) or `'hover'` (activates on mouse-over). Because DrawingML names the mouse-over hyperlink element differently per parent type, `on: 'hover'` emits `<a:hlinkMouseOver>` on text runs (`CT_TextCharacterProperties`) and `<a:hlinkHover>` on shape/image `<p:cNvPr>` (`CT_NonVisualDrawingProps`) — both the same `CT_Hyperlink` type, carrying identical attributes (`r:id`, `tooltip`, `action`, …). Hover works with `url` or `slide` and reuses the existing hyperlink relationship machinery unchanged. Default-off: hyperlinks with no `on` (or `on: 'click'`) emit byte-identical `<a:hlinkClick>` XML.
- Motion-path animation — `animation = { type: 'motionPath', path }` animates an object along a custom path via `<p:animMotion origin="layout" path="…" pathEditMode="relative">` (build step `presetClass="path"`, `presetID="0"`) inside the shared `<p:timing>` engine, targeting `ppt_x`/`ppt_y`. `path` is an SVG-like command string in **normalized 0–1 slide coordinates** (`M`/`L`/`C`/`Z`, lowercase relative variants accepted), emitted **verbatim** with an appended ` E` end marker (the path is **not** routed through the `svgPathToOoxml` `<a:custGeom>` helper). The path is validated against an allowed-token set (`/^[MLCZmlcze0-9.,+\-\s]+$/`, which also rejects XML-unsafe `<`/`>`/`&`/`"`); a missing or invalid path warns and omits the motion payload. Motion targets an already-visible object, so no entrance visibility `<p:set>` is emitted. `duration`/`delay`/`trigger`/`group`/`stagger` reuse the shared timing engine unchanged.
- Grid layout helper — `pptx.layoutGrid({ items, columns, area, gap?, gapX?, gapY?, padding?, align?, valign? })` computes evenly-spaced `{ x, y, w, h }` cell positions (inches) within a bounding `area`, returning one cell per item in item order. Pure math (no OOXML emission): `cellW = (area.w - (columns-1)*gapX) / columns`, `rows = ceil(items/columns)`, `cellH = (area.h - (rows-1)*gapY) / rows`. `gap` (default `0.2`) applies to both axes unless overridden by `gapX`/`gapY`; `padding` insets each returned cell; a partial last row honours `align` (`start`/`center`/`stretch`). Edge cases: `items === 0` → `[]`; zero/negative `area` width/height → throws. Eliminates repetitive grid math when positioning capability cards, icon grids, and comparison layouts. Also exported as `layoutGrid()` from `gen-utils`.
- Shape grouping — `slide.addGroup({ x, y, w, h })` returns a group handle whose `addShape()`/`addText()` take coordinates **relative to the group origin**, emitting a `<p:grpSp>` with `<p:grpSpPr>` carrying the absolute `<a:xfrm>` (off/ext) plus `chOff="0,0"`/`chExt` equal to the extent (1:1 child coordinate space). Eliminates manual x/y grid math when laying out clusters of shapes/text.
- Rounded-rectangle callout sugar — `slide.addCallout({ text, x, y, w, h, fill, fontColor, fontSize, fontBold, cornerRadius, align, valign })` is a convenience wrapper over a `roundRect` shape with centred text. `cornerRadius` (inches) maps to the OOXML corner-radius `adj` via `adj = Math.round((cornerRadius / (h / 2)) * 50000)`, emitted as `<a:prstGeom prst="roundRect"><a:avLst><a:gd name="adj" fmla="val N"/></a:avLst></a:prstGeom>`.
- Shadow & glow effects on shapes — `shadow = { type: 'outer', blur, offset, angle, color, opacity }` and `glow = { size, color, opacity }` emit a single `<a:effectLst>` containing `<a:outerShdw>` and/or `<a:glow>`. Unit conversions: blur/offset/size pt → EMU (×12700), angle degrees → 60,000ths, opacity decimal → thousandths-percent (×100000).
- SVG-to-shape conversion — `svgPath = { d, viewBox: { w, h } }` on `addShape()` converts an SVG `<path d="…">` to OOXML custom geometry (`<a:custGeom>`) instead of a preset geometry, so vector icons keep their fidelity rather than being rasterized to PNG. Supports `M`/`L`/`H`/`V`/`C`/`Q`/`Z` commands (absolute) and their lower-case relative variants (relative coordinates are tracked against the current pen position and converted to absolute); path coordinates are scaled from the viewBox into EMU. A reusable `svgPathToOoxml(d, width, height)` helper is exported from `gen-utils`.
...
- Auto-fit text — `fit` on `addText()` controls how text fits its shape: `'shrink'` emits `<a:normAutofit fontScale="70000"/>` (shrink text on overflow), `'resize'` emits `<a:spAutoFit/>` (resize shape to fit text), and `'none'` disables auto-fit. The legacy `autoFit: true` option now maps to `fit: 'resize'` for backwards compatibility.
...
- Shape, text, and image entrance animations — `animation = { type, duration?, delay?, trigger?, direction? }` emits a native per-slide `<p:timing>` block. Types: `appear`, `fadeIn`, `flyIn` (directional — slides in from one full slide-width/height offscreen via `#ppt_x±1slide` / `#ppt_y±1slide`), `zoomIn`. Triggers: `onClick`, `withPrevious`, `afterPrevious`.
- Animation auto-grouping sugar — `animation.group` (number) and `animation.stagger` (ms) on any animated object remove the need to assign `trigger`/`delay` by hand. Objects sharing the same `group` animate together (resolved to `withPrevious` within one build step); a change to a different `group` starts the next step (`afterPrevious`), so groups reveal sequentially. When `stagger` is set, the Nth item within a group (0-indexed) gets a cumulative `delay = N * stagger`. Resolution happens in `genXmlTiming()` before the build-step logic — pure syntactic sugar over the existing `trigger`/`delay`, emitting no new OOXML elements. Objects without a `group` keep their explicit `trigger` behaviour byte-for-byte (backwards-compatible).
- Structured card helper — `slide.addCard({ x, y, w, h, title, description?, icon?, badge?, fill?, border?, cornerRadius?, shadow?, glow?, titleFont?, descFont?, iconSize?, iconFill?, align?, iconPosition?, animation? })` emits a single shape group (`<p:grpSp>`) containing a rounded-rect background plus, as applicable, an icon container + icon (an SVG path rendered as `<a:custGeom>`, or an emoji/text glyph), a title, a shrink-to-fit description, and a top-right badge — all positioned with group-relative coordinates. An optional `animation` is attached to the group so the whole card animates as one. Default-off: a deck that never calls `addCard()` emits no extra markup. Replaces the 5–8 manual `addShape`/`addText` calls a card normally needs.
...
- Pattern (preset hatch) fills — `fill = { type: 'pattern', preset, foreColor, backColor? }` on shapes emits `<a:pattFill prst="…">` with an `<a:fgClr>` foreground and an optional `<a:bgClr>` background. `preset` is one of the 54 ECMA-376 `ST_PresetPatternVal` hatch names (e.g. `'ltUpDiag'`, `'cross'`, `'pct50'`); an unrecognised preset is skipped with a `console.warn` so output stays schema-valid (guard-don't-crash). Omitting `backColor` omits `<a:bgClr>` entirely (transparent background). Fore/back colours accept hex or `SchemeColor` values, resolved through the same color selector as solid/gradient fills. Default-off: shapes without a pattern fill emit byte-identical XML.
- Picture (image) fills — `fill = { type: 'image', path? | data?, sizing?, transparency? }` on shapes emits `<a:blipFill>` with an `<a:blip r:embed="…">` referencing a registered media relationship (a media part + slide `_rels` entry + `[Content_Types].xml` Default — reusing the `addImage()` pipeline, including path-keyed dedupe). `sizing` is `'stretch'` (default → `<a:stretch><a:fillRect/>`) or `'tile'` (→ `<a:tile tx="0" ty="0" sx="100000" sy="100000" algn="tl"/>`); optional `transparency` (0–100) emits `<a:alphaModFix>`. Missing both `path` and `data` (or an SVG source, which is not yet supported as a shape fill) falls back to `<a:noFill/>` with a `console.warn` (guard-don't-crash) and registers no dangling relationship. Default-off: shapes without an image fill emit byte-identical XML.
- Emphasis animations — `animation.type` now accepts the emphasis effects `pulse`, `spin`, `grow`, and `colorPulse` (alongside the existing entrance effects), drawing attention to an already-visible object. They reuse the existing `<p:timing>` build-step engine but emit the member `<p:cTn>` with `presetClass="emph"` (vs `"entr"`) and **omit** the leading visibility `<p:set>` (the object is already shown). `spin` emits `<p:animRot by="…">` on `r` (`spinDegrees`, default 360°, × 60000); `grow` emits `<p:animScale>` with `<p:by x/y>` (`growScale`, default 1.5×, × 100000); `colorPulse` emits `<p:animClr clrSpc="rgb">` with `<p:to><a:srgbClr val="…"/>` (`color`, default `000000`); `pulse` emits `<p:anim>` on `style.opacity` dipping 100% → 0 → 100%. Trigger/group/stagger grouping semantics are unchanged. Default-off: entrance animations and decks with no animation emit byte-identical XML.
- Exit animations — `animation.type` now accepts the exit effects `disappear`, `fadeOut`, `flyOut`, and `zoomOut` (the counterpart to the entrance effects), making an already-visible object leave. They reuse the existing `<p:timing>` build-step engine but emit the member `<p:cTn>` with `presetClass="exit"` (vs `"entr"`/`"emph"`) and **omit** the leading visibility `<p:set>` (the object is already shown). `disappear` emits `<p:set>` to `style.visibility` `hidden`; `fadeOut` emits `<p:animEffect transition="out" filter="fade">` (the reverse of `fadeIn`); `flyOut` emits `<p:anim>` on `ppt_x`/`ppt_y` translating from the shape's position out to one full slide-width/height offscreen (`#ppt_x±1slide` / `#ppt_y±1slide`, reverse of `flyIn`, reusing `direction`); `zoomOut` emits two `<p:anim>` on `ppt_w`/`ppt_h` scaling from full size down to `0` (reverse of `zoomIn`). Trigger/group/stagger grouping semantics are unchanged. Default-off: entrance/emphasis animations and decks with no animation emit byte-identical XML.
- Header/footer master config — `headerFooter` on `defineSlideMaster({ headerFooter: { slideNumber?, dateTime?, footer? } })` adds a first-class API for configuring footers/dates on a master's layout. It emits a derived `<p:hf sldNum hdr ftr dt>` on the **layout** (`makeXmlLayout`, after `<p:clrMapOvr>`) reflecting the config, plus the matching placeholders inside the layout `<p:spTree>`: a footer `<p:sp>` (`<p:ph type="ftr" sz="quarter" idx="4">` with the literal `footer` text) and a date `<p:sp>` (`<p:ph type="dt" idx="1">`). `dateTime: true`/`{ format }` emits an auto-updating `<a:fld type="…">` (default `datetimeFigureOut`); `dateTime: { value }` emits literal static text instead. The master's own hardcoded `<p:hf sldNum="0" .../>` is left untouched. Default-off: masters/decks without a `headerFooter` config emit byte-identical XML (every existing layout carries no `<p:hf>`). NOTE: per-slide show/hide and notes/handout headers (`hdr`) are not yet covered — `<p:hf>` is not valid on `CT_Slide`, so those are a separate placeholder-presence mechanism (follow-on slice).
- Per-slide header/footer override — `slide.headerFooter = { footer?, dateTime? }` adds a per-slide footer/date that emits the matching placeholders inside that slide's own `<p:spTree>` (footer `<p:sp>` `<p:ph type="ftr" sz="quarter" idx="4">` with the literal `footer` text; date `<p:sp>` `<p:ph type="dt" idx="1">`). `dateTime: true`/`{ format }` emits an auto-updating `<a:fld>`; `dateTime: { value }` emits literal static text. Because `<p:hf>` is invalid on `CT_Slide`, the per-slide path is purely placeholder-presence (the `slideNumber` flag is a no-op here — use `slide.slideNumber` for the slide-number placeholder), scoped to footer + date only; explicit hide of a footer inherited from the layout is a documented limitation. Default-off: slides without a `headerFooter` set emit byte-identical XML.
- Notes-master header/footer — `pptx.notesMaster = { slideNumber?, dateTime?, footer?, header? }` is a presentation-level setter that configures the shared notes master (`notesMaster1.xml`). It injects a derived `<p:hf sldNum hdr ftr dt>` in the correct `CT_NotesMaster` child position (after `</p:clrMap>`, before `<p:notesStyle>`) reflecting the config, and fills the existing (previously empty) header `<p:ph type="hdr">` and footer `<p:ph type="ftr" idx="4">` placeholder text via `encodeXmlEntities`. Unlike slide layouts, the notes master supports a header, so `hdr` is live. The date/slide-number `<a:fld>` placeholders are untouched (their visibility is governed by the `<p:hf>` toggle). Default-off: a presentation that never sets `notesMaster` emits a byte-identical `notesMaster1.xml`. (Handout-master headers are out of scope — there is no handout-master writer.)
- Reflection effect on shapes and images — `reflection = { blur?, distance?, size?, opacity?, fadeDirection? }` emits an `<a:reflection>` inside the shape/image `<a:effectLst>`, alongside any shadow/glow. Unit conversions: `blur`/`distance` pt → EMU (×12700), `opacity`/`size` % → thousandths (×1000), `fadeDirection` degrees → 60,000ths (×60000); `endA="300"`, `sy="-100000"`, `rotWithShape="0"` are fixed constants. Defaults (when fields omitted): `blur: 0.5, distance: 0, size: 50, opacity: 50, fadeDirection: 90`. Default-off: shapes/images without a `reflection` emit byte-identical XML.
- Soft-edge (feathered edge) effect on shapes and images — `softEdge = { radius }` emits an `<a:softEdge rad="…"/>` as the **last** child of the shape/image `<a:effectLst>` (canonical `CT_EffectList` order, after any glow/shadow/reflection). `radius` is in **inches**, converted to EMU (× 914400): `radius: 0.1` → `rad="91440"`. A `radius <= 0` omits the effect entirely (and, if it is the only effect, the `<a:effectLst>` too). Default-off: shapes/images without a `softEdge` emit byte-identical XML.
- 3-D bevel / extrusion on shapes — `bevel = { top?, bottom?, depth?, contour?, material? }` emits an `<a:scene3d>` + `<a:sp3d>` pair as siblings of `<a:effectLst>` inside the shape's `<p:spPr>` (canonical `CT_ShapeProperties` order: `…effectLst, scene3d, sp3d` — `scene3d` is emitted **before** `sp3d`). `top`/`bottom` (`{ preset?, width?, height? }`) emit `<a:bevelT>`/`<a:bevelB>` (preset default `circle`; width/height in **inches** → EMU ×914400, CT_Bevel default 76200); `depth` (`{ color?, amount? }`) emits `extrusionH` (inches→EMU) and an `<a:extrusionClr>` child; `contour` (`{ color?, width? }`) emits `contourW` and an `<a:contourClr>` child; `material` emits `prstMaterial` (one of the ECMA-376 `ST_PresetMaterialType` values). `<a:scene3d>` always emits a default camera/light rig (`orthographicFront` / `threePt` / `t`) so the 3-D renders in PowerPoint; camera/light-rig overrides and 3-D on images are out of scope. Default-off: shapes without a `bevel` (or with an empty `bevel: {}`) emit byte-identical XML.
- Theme extraction utility — new optional entry point `@jsamuel1/pptxgenjs/utils` exposing `extractThemeFromCSS(css, { presets?, defaultPreset? })`. Regex-parses CSS `:root` custom properties (no DOM) and maps known variable-name patterns to a theme palette (`bg`, `bgSecondary`, `accent`, `accentSoft`, `text`, `textSecondary`, `font`, plus extended `sky`/`green`/`orange`/`red`), falling back to a built-in `dark`/`light` (or caller-supplied) preset for any slot the CSS omits. Exact variable-name matching keeps `--bg` and `--bg-card` distinct; hex values are normalised (strip `#`, uppercase, expand 3-digit shorthand) and font families are de-quoted. Pure and dependency-free — runs in Node.js. Not part of the main `PptxGenJS` class, keeping the core library focused on OOXML generation.

### Fixed

- Shape `<a:effectLst>` emitted its children in a non-canonical order — `<a:outerShdw>` was written before `<a:glow>`, but `CT_EffectList` requires the sequence `blur, fillOverlay, glow, innerShdw, outerShdw, prstShdw, reflection, softEdge`. A shape with **both** `shadow` and `glow` therefore produced schema-invalid output. The shape effect builder now emits `glow → shadow → reflection` in canonical order. A shape with only one of the effects is byte-unchanged; only the (previously invalid) both-present case changes.
- Gradient fills emitted out-of-range stop `position`/`transparency` values verbatim (they were scaled by `× 1000` but not range-clamped), producing schema-invalid output — e.g. a stop with `position: 150, transparency: 150` emitted `<a:gs pos="150000">` and `<a:alpha val="150000"/>`, both of which violate `ST_PositiveFixedPercentage` (restricted to `[0,100000]`). Each value is now clamped into `[0,100]` before the `× 1000` scale in `genXmlGradientFill` (clamp-don't-crash): `150 → 100000`, negative `→ 0`. Valid inputs are byte-unchanged (`position: 50 → 50000`, `transparency: 40 → 40000`)
- Doughnut charts emitted an out-of-range `<c:holeSize>` verbatim from the user-supplied `holeSize` option (it was type-checked but not range-clamped), producing schema-invalid output — e.g. `holeSize: 500` emitted `<c:holeSize val="500"/>`, which violates `ST_HoleSize` (an `unsignedByte` restricted to `[10,90]`). The value is now clamped to `[10,90]` and rounded to an integer (clamp-don't-crash): `500 → 90`, `5 → 10`, `60.4 → 60`. Valid integer inputs and the default (`50`) are byte-unchanged
- Scatter charts with `showLabel: true` threw `TypeError: Cannot read properties of undefined (reading '0')` during XML generation, aborting the whole `writeFile()`/`stream()` build. Scatter/bubble series are defined by `values` (X/Y) and need not carry a `labels` array, but the scatter data-label branch in `makeChartType` dereferenced `obj.labels[0]` unconditionally. The access is now null-safe (`obj.labels?.[0]`), so a series with no `labels` simply emits no per-point custom labels instead of crashing. Labelled-scatter output (`dataLabelFormatScatter: 'custom'`/`'customXY'`) is byte-unchanged
- Scatter and bubble charts failed OOXML schema validation because `makeCatAxis` closes their (numeric) x-axis as a `<c:valAx>` but still emitted four `CT_CatAx`-only children — `<c:auto>`, `<c:lblAlgn>`, `<c:noMultiLvlLbl>`, and the conditional `<c:tickLblSkip>` — none of which are valid in `CT_ValAx` (the validator flagged the first, `<c:auto>`). Those four emits are now gated to the cat-axis/date-axis case (i.e. NOT scatter/bubble/bubble3D), so the scatter/bubble value axis validates against the OOXML schema. catAx/dateAx output (bar/line/area/radar/pie/doughnut) is byte-unchanged
- LINE and combo charts failed OOXML schema validation for two reasons in `makeChartType`: (1) `CT_LineChart` requires `<c:grouping>` (minOccurs=1) as the first child of `EG_LineChartShared`, before `<c:varyColors>`, but line charts emitted no grouping — `<c:grouping val="standard"/>` is now emitted for `LINE` ahead of `<c:varyColors>`; (2) `CT_LineSer` requires `<c:marker>` to precede `<c:dLbls>`, but the data-labels block was emitted before the marker block — the `<c:marker>` block is now emitted before `<c:dLbls>`. Area/bar/bar3D/radar output is unchanged (radar skips `dLbls`, so it was already valid)
