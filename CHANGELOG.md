...
- Card icons (`addCard()` v2) — the `icon` option now accepts a font-icon glyph (`{ char, fontFace, color? }`, e.g. Font Awesome) in addition to an SVG path or emoji string; `iconFill` accepts `'none'` or `false` to render a bare icon with no container tile; and a new `iconColor` option tints the glyph independently of the tile. Existing cards (emoji/SVG icon, no `iconColor`) are byte-for-byte unchanged. Accent bars and multi-colour SVG icons remain on the roadmap.
